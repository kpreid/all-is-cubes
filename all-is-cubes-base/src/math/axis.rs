...
impl Axis {
    ...
    #[inline]
    pub const fn positive_face(self) -> Face6 {
        match self {
            Axis::X => Face6::PX,
            Axis::Y => Face6::PY,
            Axis::Z => Face6::PZ,
    ...
    #[inline]
    pub const fn negative_face(self) -> Face6 {
        match self {
            Axis::X => Face6::NX,
            Axis::Y => Face6::NY,
            Axis::Z => Face6::NZ,
...
impl const From<Axis> for u8 {
    #[inline]
    fn from(value: Axis) -> Self {
        value as u8
    }
}
impl const From<Axis> for usize {
    #[inline]
    fn from(value: Axis) -> Self {
        value as usize
    }
...
mod impl_index_axis {
    ...
    impl<T> const ops::Index<Axis> for [T; 3] {
        type Output = T;
    ...
    impl<T> const ops::IndexMut<Axis> for [T; 3] {
        #[inline]
    ...
    macro_rules! impl_xyz_e {
        ($x:ident $y:ident $z:ident, $($type:tt)*) => {
            impl<T, U> const ops::Index<Axis> for $($type)*<T, U> {
                type Output = T;
            ...
            impl<T, U> const ops::IndexMut<Axis> for $($type)*<T, U> {
                #[inline]
                fn index_mut(&mut self, index: Axis) -> &mut Self::Output {
                    match index {
                        Axis::X => &mut self.$x,
