...
macro_rules! non_generic_impls {
    ($t:ty) => {
        ...
        const impl From<PositiveSign<$t>> for NotNan<$t> {
            #[inline]
            fn from(value: PositiveSign<$t>) -> Self {
                value.into_nn()
            }
        }
        const impl From<PositiveSign<$t>> for $t {
            #[inline]
            fn from(value: PositiveSign<$t>) -> Self {
                value.0
            }
        }
        const impl From<ZeroOne<$t>> for NotNan<$t> {
            #[inline]
            fn from(value: ZeroOne<$t>) -> Self {
                value.into_nn()
            }
        }
        const impl From<ZeroOne<$t>> for $t {
            #[inline]
            fn from(value: ZeroOne<$t>) -> Self {
                value.0
            }
        ...
        const impl From<ZeroOne<$t>> for PositiveSign<$t> {
            #[inline]
            fn from(value: ZeroOne<$t>) -> Self {
                value.into_ps()
            }
        ...
        const impl TryFrom<$t> for PositiveSign<$t> {
        ...
        const impl TryFrom<$t> for ZeroOne<$t> {
...
const impl<T> AsRef<T> for PositiveSign<T> {
    #[inline]
    fn as_ref(&self) -> &T {
        &self.0
    }
}
const impl<T> AsRef<T> for ZeroOne<T> {
    #[inline]
    fn as_ref(&self) -> &T {
        &self.0
    }
...
mod integer_to_positive_sign {
    ...
    macro_rules! integer_to_positive_sign {
        ($int:ident, $float:ident) => {
            const impl From<$int> for PositiveSign<$float> {
                #[inline]
