...
#[non_exhaustive]
pub enum Modifier {
    /// Sets or overrides a single [attribute](BlockAttributes) of the block.
    //---
    // Design note: It would be better, for efficient memory layout and to avoid arbitrarily
    // distinguishing “set exactly one attribute” and higher-level operations, this would be
    // individual variants of `Modifier` instead of a separate enum. However, making an enum whose
    // variants are *partially* generated by a `macro_rules` is hard and confuses rust-analyzer.
    SetAttribute(block::SetAttribute),
...
impl Modifier {
    ...
    pub(in crate::block) fn evaluate(
        ...
        Ok(match *self {
            Modifier::SetAttribute(ref attribute) => {
                attribute.apply(value.attributes_mut());
                value
            }
    ...
    pub(crate) fn unspecialize(&self, block: &Block) -> ModifierUnspecialize {
        ...
        match self {
            Modifier::SetAttribute(_) => ModifierUnspecialize::Keep,
    ...
    pub(crate) fn does_not_introduce_asymmetry(&self) -> bool {
        match self {
            Modifier::SetAttribute(attr) => attr.rotationally_symmetric(),
    ...
    pub(crate) fn rotate(self, rotation: GridRotation) -> Self {
        match self {
            Modifier::SetAttribute(a) => Modifier::SetAttribute(a.rotate(rotation)),
            // TODO: I am pretty sure I want tags to be able to identify faces/rotations
            // but they don't currently
            m @ Modifier::Tag(_) => m,
...
impl VisitHandles for Modifier {
    fn visit_handles(&self, visitor: &mut dyn HandleVisitor) {
        match self {
            Modifier::SetAttribute(a) => a.visit_handles(visitor),
...
mod tests {
    ...
    fn modifier_debug() {
        let modifiers: Vec<Modifier> = vec![
            Modifier::SetAttribute(block::SetAttribute::DisplayName(arcstr::literal!("hello"))),
