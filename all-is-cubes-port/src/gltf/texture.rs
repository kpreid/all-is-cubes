...
use all_is_cubes::euclid::{Point2D, Scale};
...
/// TODO: make sure this is used in all the places it should be.
...
impl UvMap {
    #[expect(dead_code, reason = "TODO: texture coordinates not implemented yet")]
    /// Placeholder value to use when there are not actually any textures
    pub(crate) fn empty() -> Self {
        Self {
            positioning: HashMap::new(),
            scale: Scale::new(0.0),
        }
    }
    ...
    pub fn plane_to_atlas(
        ...
        let atlas_origin = self
            .positioning
            .get(&plane_id)
            .copied()
            .unwrap_or_else(|| panic!("plane_id {plane_id:?} not found in atlas mapping"));
        self.scale
            .transform_point(
                atlas_origin.map(f64::from) + texel_in_plane.map(f64::from).to_vector(),
            )
            .cast::<f32>()
    }
}
