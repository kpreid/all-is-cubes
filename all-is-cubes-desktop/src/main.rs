...
fn main() -> Result<(), ()> {
    ...
    let surface = GlfwSurface::new_gl33("All is Cubes", WindowOpt::default().set_dim(dim))
        .expect("GLFW surface creation");
    let viewport = glfw_viewport(&surface);
    ...
    'app: loop {
        ...
        if resize {
            resize = false;
            renderer.set_viewport(glfw_viewport(&renderer.surface));
        }
        if update_cursor {
...
fn glfw_viewport(surface: &GlfwSurface) -> Viewport {
    Viewport {
...
fn map_glfw_button(button: glfw::MouseButton) -> usize {
...
fn map_glfw_key(key: glfw::Key) -> Option<all_is_cubes::camera::Key> {
